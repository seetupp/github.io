...
// Giriş kütüphanesi ayrı yüklenir. Yüklenemezse site yine de açılır (sadece giriş çalışmaz).
let getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, onAuthStateChanged;
const authModuleReady = import("https://www.gstatic.com/firebasejs/12.14.0/firebase-auth.js")
    .then((m) => {
        ({ getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, onAuthStateChanged } = m);
        return true;
    })
    .catch((e) => {
        console.error("Firebase Auth yüklenemedi:", e);
        return false;
    });

function getAuthInstance() {
    if (!authInstance && getAuth && window.firebaseApp) authInstance = getAuth(window.firebaseApp);
    return authInstance;
}
...
window.handleAuth = async function(type) {
    ...
    await authModuleReady;
    const auth = getAuthInstance();
    if (!auth) {
        alert("Giriş sistemi yüklenemedi. İnternetini kontrol edip sayfayı yenile.");
        return;
    }
